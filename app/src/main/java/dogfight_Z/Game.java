...
//		eventManager.setTitle("dogfight_Z");
		...
		*/
		...
		//-------------------------------------
		MouseWheelControl mouse = new MouseWheelControl(eventManager.EventFrapsQueue_keyboard);
		eventManager.mainScr.addMouseWheelListener( mouse);
		eventManager.mainScr.addMouseListener(mouse);
		eventManager.mainScr.addKeyListener(new ContinueListener(this, Thread.currentThread()));
...
//			eventManager.mainScr.setBackgroundColor( Color.argb(0XFF,R_Back, G_Back, B_Back));
	...
	public void run()
	{
		int keyPressed;
		int scrSize = 16;
		int resolution_min = Math.min(visualManager.getResolution_X(), visualManager.getResolution_Y()) / 2;
		...
		while(gameStopTime == -1    ||    (leftTime = gameStopTime - System.currentTimeMillis()/1000) > 0)
		{
			...
			*/
			...
			myJet.control_roll_lr((double)(-xy.x)/64.0);
			myJet.control_roll_up_dn((double)(-xy.y)/64.0);
			...
			while(!eventManager.EventFrapsQueue_keyboard.isEmpty())
			{
				...
				switch(keyPressed)
				{
					...
					case KeyEvent.KEYCODE_SPACE://Shift
						keyState_SHIFT = true;
					break;
					case -KeyEvent.KEYCODE_SPACE://-Shift
						keyState_SHIFT = false;
