...
public class Aircraft extends CharMessObject
{
	...
	{
		if(Math.abs(roll_angle[1]) > 90.0)
			roll_angle[0] += Math.sin(Math.toRadians(roll_angle[2])) * angleVel / 6.4;
		else
			roll_angle[0] -= Math.sin(Math.toRadians(roll_angle[2])) * angleVel / 6.4;
	...
	{
		if(Math.abs(roll_angle[1]) > 90.0)
			roll_angle[0] -= Math.cos(Math.toRadians(roll_angle[2])) * angleVel / 6.4;
		else
			roll_angle[0] += Math.cos(Math.toRadians(roll_angle[2])) * angleVel / 6.4;
